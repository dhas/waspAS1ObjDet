...
if not Path(val_h5).exists():
	val_dataset.create_hdf5_dataset(file_path=val,
	                                resize=False,
	                                variable_image_size=True,
	                                verbose=True)
...
model_checkpoint = ModelCheckpoint(filepath='./models/ssd7_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
                                   monitor='val_loss',
                                   verbose=1,
                                   save_best_only=True,
...
csv_logger = CSVLogger(filename='models/ssd7_training_log.csv',
                       separator=',',
                       append=True)
